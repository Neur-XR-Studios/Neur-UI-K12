...
public class AutomationUnitTestScript : EditorWindow
{
    ...
    /*private void RunAllChecks()
    {
        LandingPageImagePresent = CheckLandingPageImage();
        csvFilePresent = CheckCSVFile();
        if (csvFilePresent.Value)
        {
            objectContentPresent = CheckObjectContentPresence();
            if (objectContentPresent.Value)
            {
                objectInfoCountMatches = CheckObjectInfoCount();
            }
            else
            {
                objectInfoCountMatches = null; // Skip the count check if content is missing
            }
        }        
    }*/
    ...
    {
        bool fileExists = System.IO.File.Exists("Assets/Resources/Images/Icon.png");
        if (!fileExists)
        {
            if (!System.IO.File.Exists("Assets/Resources/Images/Icon.jpg"))
            {
                Debug.LogError("Icon file is missing at: Assets/Resources/Images/Icon");                
            }
            else
            {
                fileExists = true;
